...
const rule = {
    ...
    create: function (context) {
         ...
         * @returns {boolean}
         */
        function isAsyncCallExpressionLocal(node) {
            const assertContextVar =
                assertVariableStack[assertVariableStack.length - 1];
            if (!assertContextVar) {
                return false;
            }
            return isAsyncCallExpression(node, assertContextVar);
        }
        ...
        function getAsyncCallType(node) {
            ...
            if (isAsyncCallExpressionLocal(node)) {
                const assertContextVar =
                    assertVariableStack[assertVariableStack.length - 1];
                callType = `${assertContextVar}.async()`;
            } else if (node.type === "CallExpression" && isStop(node.callee)) {
                callType = "stop()";
            } else if (node.type === "CallExpression" && isStart(node.callee)) {
                callType = "start()";
            }
            ...
            CallExpression: function (node) {
                ...
                if (isTest(node.callee)) {
                    ...
                    const isStopOrStartOrAsync =
                        isAsyncCallExpressionLocal(node) ||
                        isStop(node.callee) ||
                        isStart(node.callee);
