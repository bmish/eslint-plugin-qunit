...
const rule = {
    ...
    create: function (context) {
        ...
        /** @type {Array<{returnAndAssertNodes: import('eslint').Rule.Node[]}>} */
        ...
        function popFunction() {
            if (assertContextVar !== null) {
                ...
                for (
                    i = lastScope.returnAndAssertNodes.length - 1;
                    i >= 0;
                    --i
                ) {
                    if (
                        lastScope.returnAndAssertNodes[i].type ===
                        "CallExpression"
                    ) {
                        lastAssert = i;
                        break;
                ...
                if (
                    lastAssert !== null &&
                    lastScope.returnAndAssertNodes.length > 0
                ) {
                    for (const node of lastScope.returnAndAssertNodes.slice(
                        0,
                        lastAssert,
                    )) {
        ...
        return {
            CallExpression: function (node) {
                if (isTest(node.callee)) {
                    ...
                    assertContextVar &&
                    isAssertion(node.callee, assertContextVar) &&
                    functionScopes.length > 0
                ) {
                    functionScopes[
                        functionScopes.length - 1
                    ].returnAndAssertNodes.push(node);
                }
            },
